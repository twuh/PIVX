...
- [Development guidelines](#development-guidelines)
    - [General WORX Core](#general-worx-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
WORX Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play WORXs (tWORX)" on the test network, if you
...
WORX Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/worx-project/worx/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_worx
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_worx --log_level=test_suite
$ valgrind -v --leak-check=full src/worxd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_worx.coverage/index.html`.
```
...
- BitcoinMiner : Generates WORXs (if wallet is enabled).
...
However, in open source software such as WORX Core, where everyone uses
...
pay attention to for reviewers of WORX Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: WORX Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against WORX Core so that they can be integrated
...
file descriptors it will cause problems with WORX's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:WORX-Project/WORX.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `worx-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `worx-cli` and the GUI debug console use this table to determine how to
