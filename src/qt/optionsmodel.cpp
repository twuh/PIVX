...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset){
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::WORX);
    ...
    if (settings.contains("nAnonymizeWorxxAmount") || reset)
        SoftSetArg("-anonymizeworxamount", settings.value("nAnonymizeWorxxAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in worx.cpp during shotdown to also remove the window positions
