...
#include "qt/worx/forms/ui_addresseswidget.h"
#include "qt/worx/addresslabelrow.h"
#include "qt/worx/addnewaddressdialog.h"
#include "qt/worx/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(WORXGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter a WORX address"));
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress worxAdd = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(worxAdd.Get(), label.toUtf8().constData(), "send")) {
