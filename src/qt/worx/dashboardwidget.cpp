...
#include "qt/worx/dashboardwidget.h"
#include "qt/worx/forms/ui_dashboardwidget.h"
#include "qt/worx/sendconfirmdialog.h"
#include "qt/worx/txrow.h"
...
DashboardWidget::DashboardWidget(WORXGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of WORX and zWORX staked."));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquareWorx, "square-chart-worx");
    setCssProperty(ui->labelSquarezWorx, "square-chart-zworx");
    setCssProperty(ui->labelWorx, "text-chart-worx");
    setCssProperty(ui->labelZworx, "text-chart-zworx");
    ...
    ui->labelAmountZworx->setText("0 zWORX");
    ui->labelAmountWorx->setText("0 WORX");
    setCssProperty(ui->labelAmountWorx, "text-stake-worx-disable");
    setCssProperty(ui->labelAmountZworx, "text-stake-zworx-disable");
    ...
    ui->btnHowTo->setText(tr("How to get WORX or zWORX"));
...
void DashboardWidget::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("WORX")
    updateDisplayUnit();
}
...
QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy() {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isWorx = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZWORX;
...
void DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    chartData->amountsByCache = getAmountBy(); // pair WORX, zWORX
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            worx = (pair.first != 0) ? pair.first / 100000000 : 0;
            zworx = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalWorx += pair.first;
            chartData->totalZworx += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesWorx.append(worx);
        chartData->valueszWorx.append(zworx);

        int max = std::max(worx, zworx);
...
void DashboardWidget::onChartRefreshed() {
    ...
    // init sets
    set0 = new QBarSet("WORX");
    set1 = new QBarSet("zWORX");
    ...
    set0->append(chartData->valuesWorx);
    set1->append(chartData->valueszWorx);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalWorx > 0 || chartData->totalZworx > 0) {
        setCssProperty(ui->labelAmountWorx, "text-stake-worx");
        setCssProperty(ui->labelAmountZworx, "text-stake-zworx");
    } else {
        setCssProperty(ui->labelAmountWorx, "text-stake-worx-disable");
        setCssProperty(ui->labelAmountZworx, "text-stake-zworx-disable");
    }
    forceUpdateStyle({ui->labelAmountWorx, ui->labelAmountZworx});
    ui->labelAmountWorx->setText(GUIUtil::formatBalance(chartData->totalWorx, nDisplayUnit));
    ui->labelAmountZworx->setText(GUIUtil::formatBalance(chartData->totalZworx, nDisplayUnit, true));
