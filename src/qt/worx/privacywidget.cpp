...
#include "qt/worx/privacywidget.h"
#include "qt/worx/forms/ui_privacywidget.h"
#include "qt/worx/qtutils.h"
#include "guiutil.h"
#include "qt/worx/denomgenerationdialog.h"
#include "qt/worx/txviewholder.h"
...
PrivacyWidget::PrivacyWidget(WORXGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zWORX anonymizes your WORX by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zWORX or convert back to WORX"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 WORX ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zWORX");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zWORX");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zWORX");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zWORX");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zWORX");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zWORX");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zWORX");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zWORX");
    ...
    // Only Convert to WORX enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzWORX->setTitleClassAndText("btn-title-grey", "Total 0 zWORX");
    ui->btnTotalzWORX->setSubTitleClassAndText("text-subtitle", "Show denominations of zWORX owned.");
    ui->btnTotalzWORX->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select WORX outputs to mint into zWORX.");
    ...
    connect(ui->btnTotalzWORX, SIGNAL(clicked()), this, SLOT(onTotalZworxClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zWORX");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of WORX to mint into zWORX"));
    }else{
        btnText = tr("Convert back to WORX");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zWORX to convert back into WORX"));
    }
...
void PrivacyWidget::onTotalZworxClicked(){
    bool isVisible = ui->layoutDenom->isVisible();
    if(!isVisible){
        ui->layoutDenom->setVisible(true);
        ui->btnTotalzWORX->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzWORX->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zWORX is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zWORX, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zWORX").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zWORX minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZworx(
            value,
            selectedMints,
            mintChange,
            minimizeChange,
        ...
        // Spend succeed
        inform(tr("zWORX converted back to WORX"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    ui->btnTotalzWORX->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
