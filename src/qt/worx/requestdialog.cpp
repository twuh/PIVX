...
#include "qt/worx/requestdialog.h"
#include "qt/worx/forms/ui_requestdialog.h"
#include <QListView>
    ...
    ui(new Ui::RequestDialog)
{
    ...
    ui->labelMessage->setText(tr("Instead of share a WORX address, you can create a Payment Request message which bundles up more information than is contained in just a WORX address."));
    ...
    ui->comboBoxCoin->addItem("WORX", 0);
    ui->comboBoxCoin->addItem("zWORX", 1);
...
void RequestDialog::onNextClicked(){
    if(walletModel) {
        ...
        ui->labelTitle->setText("Request for " + BitcoinUnits::format(displayUnit, value, false, BitcoinUnits::separatorAlways) + " WORX");
