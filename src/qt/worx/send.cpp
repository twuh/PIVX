...
#include "qt/worx/send.h"
#include "qt/worx/forms/ui_send.h"
#include "qt/worx/addnewcontactdialog.h"
#include "qt/worx/qtutils.h"
#include "qt/worx/sendchangeaddressdialog.h"
#include "qt/worx/optionbutton.h"
#include "qt/worx/sendconfirmdialog.h"
#include "qt/worx/myaddressrow.h"
...
SendWidget::SendWidget(WORXGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Button Group */
    ui->pushLeft->setText("WORX");
    setCssProperty(ui->pushLeft, "btn-check-left");
    ui->pushLeft->setChecked(true);
    ui->pushRight->setText("zWORX");
    setCssProperty(ui->pushRight, "btn-check-right");

    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (WORX) or private coins (zWORX)"));
    ...
    /* Address */
    ui->labelSubtitleAddress->setText(tr("Enter a WORX address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 WORX");
    ...
    setCssProperty(coinIcon, "coin-icon-worx");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onWORXSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onWORXSelected(false);});
...
void SendWidget::refreshView(){
    ...
    if(ui->pushLeft->isChecked()){
        btnText = tr("Send WORX");
        ui->pushButtonAddRecipient->setVisible(true);
    }else{
        btnText = tr("Send zWORX");
...
void SendWidget::refreshAmounts() {
    ...
    bool isZworx = ui->pushRight->isChecked();
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();

    ui->labelAmountSend->setText(GUIUtil::formatBalance(total, nDisplayUnit, isZworx));
    ...
    if (CoinControlDialog::coinControl->HasSelected()){
        ...
        // Wallet's balance
        totalAmount = (isZworx ? walletModel->getZerocoinBalance() : walletModel->getBalance()) - total;
...
void SendWidget::onSendClicked(){
    ...
    bool sendWorx = ui->pushLeft->isChecked();

    // request unlock only if was locked or unlocked for mixing:
    // this way we let users unlock by walletpassphrase or by menu
    // and make many transactions while unlocking through this dialog
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendWorx ? AskPassphraseDialog::Context::Send_WORX : AskPassphraseDialog::Context::Send_zWORX, true)){
        // Unlock wallet was cancelled
        inform(tr("Cannot send, wallet locked"));
        return;
    }

    if((sendWorx) ? send(recipients) : sendZworx(recipients)) {
...
bool SendWidget::sendZworx(QList<SendCoinsRecipient> recipients){
    if (!walletModel || !walletModel->getOptionsModel())
        return false;
    ...
    if (walletModel->sendZworx(
        ...
        inform(tr("zWORX transaction sent!"));
        ZWorxControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == ZWORX_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zWORX require a security level of 100 to successfully spend.");
        } else {
        ...
        emit message("zWORX transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isWORX){
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any WORX to select."));
        }
    }else{
        if (walletModel->getZerocoinBalance() > 0) {
            ...
            zWorxControl->exec();
            ui->btnCoinControl->setActive(!ZWorxControlDialog::setSelectedMints.empty());
            zWorxControl->deleteLater();
        } else {
            inform(tr("You don't have any zWORX in your balance to select."));
        }
    }
}
...
void SendWidget::onWORXSelected(bool _isWORX){
    isWORX = _isWORX;
    setCssProperty(coinIcon, _isWORX ? "coin-icon-worx" : "coin-icon-zworx");
    refreshView();
...
void SendWidget::onContactMultiClicked(){
    if(focusedEntry) {
        ...
        CBitcoinAddress worxAdd = CBitcoinAddress(address.toStdString());
        if (walletModel->isMine(worxAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(worxAdd.Get(), dialog->getLabel().toStdString(), "send")) {
