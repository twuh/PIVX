...
#include "qt/worx/topbar.h"
#include "qt/worx/forms/ui_topbar.h"
#include <QPixmap>
#include "qt/worx/lockunlock.h"
#include "qt/worx/qtutils.h"
#include "qt/worx/receivedialog.h"
...
TopBar::TopBar(WORXGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopWorx, ui->labelAmountTopzWorx}, "amount-small-topbar");
    setCssProperty({ui->labelAmountWorx, ui->labelAmountzWorx}, "amount-topbar");
    setCssProperty({ui->labelPendingWorx, ui->labelPendingzWorx, ui->labelImmatureWorx, ui->labelImmaturezWorx}, "amount-small-topbar");
...
void TopBar::updateBalances(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance,
    ...
    CAmount worxAvailableBalance = balance - immatureBalance - nLockedBalance;

    // zWORX Balance
    CAmount matureZerocoinBalance = zerocoinBalance - unconfirmedZerocoinBalance - immatureZerocoinBalance;

    // Set
    QString totalWorx = GUIUtil::formatBalance(worxAvailableBalance, nDisplayUnit);
    QString totalzWorx = GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true);
    // Top
    ui->labelAmountTopWorx->setText(totalWorx);
    ui->labelAmountTopzWorx->setText(totalzWorx);

    // Expanded
    ui->labelAmountWorx->setText(totalWorx);
    ui->labelAmountzWorx->setText(totalzWorx);

    ui->labelPendingWorx->setText(GUIUtil::formatBalance(unconfirmedBalance, nDisplayUnit));
    ui->labelPendingzWorx->setText(GUIUtil::formatBalance(unconfirmedZerocoinBalance, nDisplayUnit, true));

    ui->labelImmatureWorx->setText(GUIUtil::formatBalance(immatureBalance, nDisplayUnit));
    ui->labelImmaturezWorx->setText(GUIUtil::formatBalance(immatureZerocoinBalance, nDisplayUnit, true));
}
