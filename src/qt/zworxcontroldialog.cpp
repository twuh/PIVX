...
#include "ui_zworxcontroldialog.h"
...
std::set<std::string> ZWorxControlDialog::setSelectedMints;
std::set<CMintMeta> ZWorxControlDialog::setMints;

bool CZWorxControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZWorxControlDialog::COLUMN_DENOMINATION || column == ZWorxControlDialog::COLUMN_VERSION || column == ZWorxControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZWorxControlDialog::ZWorxControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZWorxControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
    ...
    // Title
    ui->labelTitle->setText(tr("Select zWORX Denominations to Spend"));
    ui->labelTitle->setProperty("cssClass", "text-title-dialog");


    // Label Style
    ui->labelZWorx->setProperty("cssClass", "text-main-purple");
    ui->labelZWorx_int->setProperty("cssClass", "text-main-purple");
...
ZWorxControlDialog::~ZWorxControlDialog()
{
    delete ui;
}

void ZWorxControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZWorxControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZWorxControlWidgetItem* itemDenom(new CZWorxControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZWorxControlWidgetItem *itemMint = new CZWorxControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zworxTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zworxTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zWORX.";
            else if (!mint.isSeedCorrect)
                strReason = "The zWORX seed used to mint this zWORX is not the same as currently hold in the wallet";
            else
...
void ZWorxControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZWorxControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZWorx_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    //privacyDialog->setZWorxControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZWorxControlDialog::GetSelectedMints()
{
...
void ZWorxControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
