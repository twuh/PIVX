...
void ProcessMessageSwiftTX(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "ix") {
        ...
        if (fAccepted) {
            ...
            if (GetTransactionLockSignatures(tx.GetHash()) == INSTANTX_SIGNATURES_REQUIRED) {
    ...
    } else if (strCommand == "txlvote") // SwiftX Lock Consensus Votes
    {
        ...
        if (mapTxLockReq.count(ctx.txHash) && GetTransactionLockSignatures(ctx.txHash) == INSTANTX_SIGNATURES_REQUIRED) {
...
int GetTransactionLockSignatures(uint256 txHash)
{
    ...
    if (!IsSporkActive(SPORK_2_INSTANTX)) return -1;
