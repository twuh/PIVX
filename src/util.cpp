...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "worx" is a composite category enabling all WORX-related debug output
            if (ptrCategory->count(std::string("worx"))) {
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\WORX
// Windows >= Vista: C:\Users\Username\AppData\Roaming\WORX
// Mac: ~/Library/Application Support/WORX
// Unix: ~/.worx
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "WORX";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "worx.conf"));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty worx.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override worx.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "worxd.pid"));
