...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that zworxspends need to use the cs_spendcache
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 WORX at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zWORX send
enum ZerocoinSpendStatus {
    ZWORX_SPEND_OKAY = 0,                            // No error
    ZWORX_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZWORX_WALLET_LOCKED = 2,                         // Wallet was locked
    ZWORX_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZWORX_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZWORX_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZWORX_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZWORX_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZWORX_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZWORX_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZWORX_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZWORX_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZWORX_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZWORX_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZWORX_SPENT_USED_ZWORX = 14,                      // Coin has already been spend
    ZWORX_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZWORX_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZWorxBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZWORXOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    std::string GetUniqueWalletBackupName(bool fzworxAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzWORXTracker> zworxTracker;
    ...
    void setZWallet(CzWORXWallet* zwallet)
    {
        zwalletMain = zwallet;
        zworxTracker = std::unique_ptr<CzWORXTracker>(new CzWORXTracker(strWalletFile));
    }

    CzWORXWallet* getZWallet() { return zwalletMain; }
    ...
    void setZWorxAutoBackups(bool fEnabled)
    {
    ...
    /** zWORX reset */
    boost::signals2::signal<void()> NotifyzWORXReset;
