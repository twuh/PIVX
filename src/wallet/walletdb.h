...
public:
    ...
    bool WriteZWORXSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZWORXSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZWORXSeed_deprecated(uint256& seed);
    bool EraseZWORXSeed();
    bool EraseZWORXSeed_deprecated();

    bool WriteZWORXCount(const uint32_t& nCount);
    bool ReadZWORXCount(uint32_t& nCount);
