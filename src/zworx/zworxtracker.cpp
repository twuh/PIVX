...
CzWORXTracker::CzWORXTracker(std::string strWalletFile)
{
...
CzWORXTracker::~CzWORXTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzWORXTracker::Init()
{
...
bool CzWORXTracker::Archive(CMintMeta& meta)
{
...
bool CzWORXTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzWORXTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzWORXTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzWORXTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzWORXTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzWORXTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzWORXTracker::GetSerialHashes()
{
...
CAmount CzWORXTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzWORXTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzWORXTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzWORXTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzWORXTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzWORXTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzWORXTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzWORXTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzWORXTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzWORXTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzWORXTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzWORXWallet* zWORXWallet)
{
    bool iszWORXWalletInitialized = (NULL != zWORXWallet);
    CMintMeta meta;
    ...
    if (! iszWORXWalletInitialized)
        zWORXWallet = new CzWORXWallet(strWalletFile);
    meta.isSeedCorrect = zWORXWallet->CheckSeed(dMint);
    if (! iszWORXWalletInitialized)
        delete zWORXWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzWORXTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzWORXTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzWORXTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzWORXTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzWORXTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzWORXTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzWORXWallet* zWORXWallet = new CzWORXWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zWORXWallet);
        }
        delete zWORXWallet;
        LogPrint("zero", "%s: added %d dzworx from DB\n", __func__, listDeterministicDB.size());
    }
