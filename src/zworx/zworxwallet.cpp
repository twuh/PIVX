...
CzWORXWallet::CzWORXWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zworx seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZWORXSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZWORXSeed_deprecated()) {
                    LogPrintf("%s: Updated zWORX seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zworx seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zworx wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzWORXWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZWORXCount(nCountLastUsed);
    else if (!walletdb.ReadZWORXCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzWORXWallet::Lock()
{
    seedMaster = 0;
}

void CzWORXWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzWORXWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZWORX(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzWORXWallet::LoadMintPoolFromDB()
{
...
void CzWORXWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzWORXWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzWORXWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zworxTracker->HasPubcoinHash(pMint.first)) {
...
bool CzWORXWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZWORX(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zworxTracker which also adds to database
    pwalletMain->zworxTracker->Add(dMint, true);
...
void CzWORXWallet::SeedToZWORX(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzWORXWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzWORXWallet::GenerateDeterministicZWORX(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzWORXWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzWORXWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzWORXWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
